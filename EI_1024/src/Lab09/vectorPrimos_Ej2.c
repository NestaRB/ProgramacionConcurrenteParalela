...
int main( int argc, char *argv[] ) {
  ...
  if (miId == 0) {
    int procActivos = numProcs-1;
    int indice = 0;

    while (procActivos > 0) {
      MPI_Recv(&peticion, 1, MPI_CHAR, MPI_ANY_SOURCE, 11, MPI_COMM_WORLD, &s);

      int destino = s.MPI_SOURCE;

      if (indice < dimVectorNumeros) {
        num = vectorNumeros[ indice ];
        MPI_Ssend(&num, 1, MPI_LONG_LONG_INT, destino, 55, MPI_COMM_WORLD);
        indice++;
      }
      else {
        num = vectorNumeros[0];
        MPI_Ssend(&num, 1, MPI_LONG_LONG_INT, destino, 44, MPI_COMM_WORLD);
        procActivos--;
      }
    }
  }
    ...
    while (1) {
      MPI_Ssend(&peticion, 1, MPI_CHAR, 0, 11, MPI_COMM_WORLD);
