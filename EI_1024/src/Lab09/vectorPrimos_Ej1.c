...
int main( int argc, char *argv[] ) {
  ...
  if (miId == 0) {
    for ( i = 0; i < dimVectorNumeros; i++ ) {
      num = vectorNumeros[ i ];
      MPI_Send(&num, 1, MPI_LONG_LONG_INT, 1, 22, MPI_COMM_WORLD);
    }
    MPI_Send(&num, 1, MPI_LONG_LONG_INT, 1, 33, MPI_COMM_WORLD);

    MPI_Recv(&numPrimosPar, 1, MPI_INT, 1, 11, MPI_COMM_WORLD, &s);
  }
  ...
  if (miId == 1) {
    ...
    MPI_Send(&numPrimosParLocal, 1, MPI_INT, 0, 11, MPI_COMM_WORLD);
  }
