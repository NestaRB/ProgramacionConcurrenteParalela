            ...
            public void actionPerformed( ActionEvent e ) {
                ...
                try {
                    ...
                    if( ( 0.0 <= ang )&&( ang < 90 )&&( vel > 0 ) ) {
                        ...
                        MiHebraCalculadoraUnDisparo t = new MiHebraCalculadoraUnDisparo(cnvCampoTiro, txfInformacion, objetivo, new NuevoDisparo(vel, ang));
                        t.start();
                    } else {
...
class MiHebraCalculadoraUnDisparo extends Thread
{
    ...
    MiHebraCalculadoraUnDisparo(CanvasCampoTiro canvas, JTextField txfInformacion, Point objetivo, NuevoDisparo nuevoDisparo)
    {
        this.canvas = canvas;
        this.txfInformacion = txfInformacion;
        this.objetivo = objetivo;
        this.nuevoDisparo = nuevoDisparo;
    }
    public void creaYMueveProyectil( NuevoDisparo d ) {
        Proyectil  p;
        boolean      impactado;

        p = new Proyectil( d.velocidadInicial, d.anguloInicial, this.canvas );
        impactado = false;
        while( ! impactado ) {
    ...
    public void run()
    {
        creaYMueveProyectil( nuevoDisparo );
    }
}
